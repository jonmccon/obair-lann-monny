---
title: Early Work
description: Student and independent work from 2009–2011 — WWU, Bellingham, and the Pacific Northwest
category: WWU
date: 2011-09-01
thumbnail: /img/thumbnails/early-work-thumb.png
tags:
  - print
  - web
  - identity
  - photography
images:
- src: "content/design/early-work/TFiF-01.png"
  alt: This Film is Free — packaging based on a Fedex dieline
- src: "content/design/early-work/TFiF-02.png"
  alt: This Film is Free project image
- src: "content/design/early-work/TFiF-03.png"
  alt: This Film is Free project image
- src: "content/design/early-work/TFiF-06.png"
  alt: This Film is Free project image
- src: "content/design/early-work/bs-01.png"
  alt: Blackspot Collective 4 — screen printing art show
- src: "content/design/early-work/bs-02.png"
  alt: Blackspot Collective 4 project image
- src: "content/design/early-work/bs-03.png"
  alt: Blackspot Collective 4 project image
- src: "content/design/early-work/dD09-01.jpeg"
  alt: WWU Design Day 09 — campus workshops
- src: "content/design/early-work/dD09-02.jpeg"
  alt: WWU Design Day 09 project image
- src: "content/design/early-work/dD09-03.jpeg"
  alt: WWU Design Day 09 project image
- src: "content/design/early-work/un-01.png"
  alt: Unosolo — pasta packaging
- src: "content/design/early-work/un-02.png"
  alt: Unosolo packaging detail
- src: "content/design/early-work/sim-01.png"
  alt: Erik Simkins Photography — Processing histogram algorithm
- src: "content/design/early-work/thisUs-01.png"
  alt: This is Us — Tumblr design for Portland director
---

A collection of work from Western Washington University and the Pacific Northwest art scene, 2009–2011.

**This Film is Free (2010)** — After finishing BA courses ahead of schedule, an independent study year exploring traditional and new media. In collaboration with photographer Erik Simkins, we gave participants a single roll of 35mm film, a series of themes and constraints, and a shared cataloging site. Packaging built from a Fedex dieline with my own photography. WordPress theme built on the Grid-a-Licious framework.

**Blackspot Collective 4 (2011)** — A screen-printing atelier and art show series in Bellingham and Georgetown. Four shows of loosely themed and hand-printed work, culminating in a former antique storefront on a corner of Georgetown.
*With: Ally Creighton, Nicole Mills, Glenn Newcomer, Battershell, Riley Hoonan, Erik Simkins, Sean Morrow.*

**WWU Design Day 09 (2009)** — Campus design workshops and fountain installations.

**Unosolo (2009)** — Packaging 101: given a gallon of dry spaghetti, figure it out. Single-serve pasta in boil-away rice paper with soy ink.

**Erik Simkins Photography (2010)** — Processing algorithm compiling an entire photography show into varying histogram channels as a simple promotional piece.

**This is Us (2009)** — Tumblr theme design and PHP/CSS/HTML skinning for a Portland-based director who had strung together production crews across multiple cities shooting the same film.

<div class="two-column">

{% image "./TFiF-01.png", "This Film is Free — packaging" %}
{% image "./TFiF-02.png", "This Film is Free project image" %}
{% image "./TFiF-03.png", "This Film is Free project image" %}
{% image "./TFiF-06.png", "This Film is Free project image" %}
{% image "./bs-01.png", "Blackspot Collective 4 — screen printing art show" %}
{% image "./bs-02.png", "Blackspot Collective 4 project image" %}
{% image "./bs-03.png", "Blackspot Collective 4 project image" %}
{% image "./bs-04.png", "Blackspot Collective 4 project image" %}
{% image "./dD09-01.jpeg", "WWU Design Day 09" %}
{% image "./dD09-02.jpeg", "WWU Design Day 09 project image" %}
{% image "./dD09-03.jpeg", "WWU Design Day 09 project image" %}
{% image "./un-01.png", "Unosolo pasta packaging" %}
{% image "./un-02.png", "Unosolo packaging detail" %}
{% image "./sim-01.png", "Erik Simkins Photography" %}
{% image "./thisUs-01.png", "This is Us — Tumblr design" %}

</div>
